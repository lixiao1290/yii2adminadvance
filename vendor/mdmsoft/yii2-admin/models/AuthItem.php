...
class AuthItem extends Model
{
    ...
    public function getItems($filter=null)
    {
        ...
        foreach ($manager->getPermissions() as $item) {//var_dump($name);exit;
            $avaliable[$item->name.'|'.$item->description] = $item->name[0] == '/' ? 'route' : 'permission';
        }
        $assigned = [];//var_dump('<pre>',$manager->getChildren($this->_item->name));exit;
        foreach ($manager->getChildren($this->_item->name) as $item) {
            $assigned[$item->name.'|'.$item->description] = $item->type == 1 ? 'role' : ($item->name[0] == '/' ? 'route' : 'permission');
            unset($avaliable[$item->name.','.$item->description]);
        }
        unset($avaliable[$this->name]);
       // var_dump('<pre>',$avaliable);exit; 
      if($filter) {
          $avaliable=   array_filter($avaliable,function($item) {
              if($item=='route') {
                  return false;
              } else {
                  return true;
